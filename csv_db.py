...
with open('oneplus_phone.csv', 'r', encoding='utf-8') as f:
    # Read the CSV data using a DictReader object
    reader = csv.DictReader(f)
    # Insert data into the table
    for row in reader:
        values = (row['\ufeffPhone_name'], row['Phone_Image'], row['Price'], row['Score'], row['Network'], row['Technical'], row['Storage'], row['Battery'], row['Display'], row['Camera'], row['OS'], row['Extra'])
